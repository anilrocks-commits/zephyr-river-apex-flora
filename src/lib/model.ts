...
export function predictNextSquad(program: Program, teamSize = 5): PredictedSquad | null {
  ...
  const ranked = metrics.map((m) => {
    ...
    let windowToPar = 0;
    let windowRounds = 0;
    const rolesInWindow: Array<"team" | "ind" | "dnp"> = [];
    for (const e of window) {
      const row = e.scores.find((s) => s.playerId === m.player.id);
      if (!row) {
        rolesInWindow.push("dnp");
        continue;
      }
      if (row.role === "dnp") {
        rolesInWindow.push("dnp");
      ...
      rolesInWindow.push(row.role === "ind" ? "ind" : "team");
    ...
    let trajectory = 0;
    for (let i = 1; i < rolesInWindow.length; i++) {
      const a = rolesInWindow[i - 1];
      const b = rolesInWindow[i];
      if ((a === "ind" || a === "dnp") && b === "team") trajectory += 12;
      else if (a === "team" && b === "ind") trajectory -= 10;
      else if (a === "team" && b === "dnp") trajectory -= 14;
      else if (a === "team" && b === "team") trajectory += 4;
    }
