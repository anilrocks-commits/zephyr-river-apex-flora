...
function isGarbageLive(live: LiveTournament): boolean {
  ...
  // Pure-future empty boards should not become score cards
  const hasScores =
...
export function liveEventsForTeam(team: LiveTeam | undefined): LiveTournament[] {
  ...
  const merged = scheduled.map((s) => {
    ...
    // Prefer clean schedule meta; never let garbage page name/venue override
    let over = overlayPage(s, page);
    if (page && isGarbageLive(page) && !(page.players?.length) && page.teamTotal == null) {
      // Keep schedule entry (name/dates from schedule) but drop page noise
      over = {
        ...s,
    ...
    if (isGarbageLive(over) && !(over.players?.length) && over.teamTotal == null) {
      // Still garbage after overlay — only keep if schedule has a real name
